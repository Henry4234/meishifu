...
if ! "${GCLOUD_BIN}" secrets describe "${RESEND_API_KEY_SECRET}" \
  --project "${PROJECT_ID}" >/dev/null 2>&1; then
  echo "Secret ${RESEND_API_KEY_SECRET} does not exist; provision it with deploy.sh first." >&2
  exit 1
fi
if ! "${GCLOUD_BIN}" tasks queues describe "${MAIL_TASKS_QUEUE}" \
  --project "${PROJECT_ID}" --location "${REGION}" >/dev/null 2>&1; then
  echo "Cloud Tasks queue ${MAIL_TASKS_QUEUE} does not exist; provision it with deploy.sh first." >&2
  exit 1
fi
